...
function Remove-FolderItems {
    ...
    Write-Host  "Remove-FolderItems $folderName starting"
    write-host "-vorher------------"
    Get-DirectoryTreeSize -Path $folderName -Recurse
    
    Remove-Item $foldername\* -Recurse -ErrorAction SilentlyContinue
    write-host "nachher -----------"
    Get-DirectoryTreeSize -Path $folderName -Recurse

}

#C:\Users\wiesner\AppData\Local\Temp
Remove-FolderItems($env:TEMP)  
$webSiteCache = "$env:LOCALAPPDATA\Microsoft\WebSiteCache"  #C:\Users\wiesner\AppData\Local
Remove-FolderItems($webSiteCache)  

$profilTemp = "$env:LOCALAPPDATA\Temp"  #C:\Users\wiesner\AppData\Local
Remove-FolderItems($profilTemp)  

$systemTemp = "$env:SystemRoot\TEMP"
Remove-FolderItems($systemTemp)  